	...
	<div class="col-sm-6 col-md-4 col-lg-3">
		<p>${success}
				<div class="card">
						...
						<div class="card-body">
						<p><a href="${pageContext.request.contextPath}/admin/category/add"><button type="button" class="btn btn-block btn-primary">Add Category</button></a><p>
						<p><a href="${pageContext.request.contextPath}/admin"><button type="button" class="btn btn-block btn-dark">Back to Admin</button></a></p>
						</div>
		...
		<c:forEach items="${categories}" var="categories">
			<div class="col-sm-6 col-md-4 col-lg-3 align-self-end">
				<div class="card" style="margin-bottom: 10px; border: none; padding-top: 10px">
						...
						<p>Created: ${categories.createdAt}</p>
						<p>Updated: ${categories.updatedAt}</p>
						<p><a href="<spring:url value="/admin/category/manage?id=${categories.categoryId}&page=${pagenumber}" />"><button type="button" class="btn btn-info">Manage</button></a></p>						
						</div>
				</div>
			</div>
		</c:forEach>
	...
	<div class="container">
			<ul class="pagination">
				 <c:if test="${pagenumber > 1}">
				  <li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/admin/category?page=${pagenumber - 1}">Previous</a></li>
				</c:if>
				 	<li class="page-item active"><a class="page-link" href="${pageContext.request.contextPath}/admin/category?page=${pagenumber}">${pagenumber}</a></li>
					<c:if test="${pagenumber + 1 <= maxpages}">
				 	<li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/admin/category?page=${pagenumber + 1}">${pagenumber + 1}</a></li>
					</c:if>
					<c:if test="${pagenumber + 2 <= maxpages}">
				 	<li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/admin/category?page=${pagenumber + 2}">${pagenumber + 2}</a></li>
					</c:if>
				<c:if test="${pagenumber + 1 <= maxpages}">
				  <li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/admin/category?page=${pagenumber + 1}">Next</a></li>
				</c:if>
				</ul>
				
				<ul class="pagination">
  					  <c:if test="${pagenumber <= maxpages && pagenumber !=1}">
				  <li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/admin/category?page=1">First</a></li>
				</c:if>
				<c:if test="${pagenumber < maxpages && maxpages != 1}">
				  <li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/admin/category?page=${maxpages}">Last</a></li>
				</c:if>
				</ul>

...
<section class="container">
	...
	<p>${success}</p>
				<img src= ${ category.photoUrl}  alt="Category Photo" width="100%" style="object-fit: contain; margin-top: 10px;">
			</div>
		
		<div class="col-sm-12 col-md-12 col-lg-4" style="margin-top: 10px; padding-bottom: 10px">
			<div class="card">
						<h3 class="card-header" style="text-align: center; border: 1px solid silver">${category.title}</h3>
						...
						<p>Photo Location: ${category.photoUrl}</p>
						<p>Photo Version: ${category.photoVersion}</p>
						<p>Photo Public ID: ${category.publicId}</p>
						<p><a href="${pageContext.request.contextPath}/admin/category/edit?id=${category.categoryId }&page=${pagenumber}"> <button type="button" class="btn btn-block btn-warning">Edit</button></a></p>
						<form method="POST" action="/admin/category/delete">
							<input type="hidden" id="id" name="id" value="${category.categoryId}" />
							<input type="submit" id="btnAdd" class="btn btn-block btn-danger" value ="Delete" onclick="return confirm('WARNING: Deleting a Category will delete ALL Products associated. Are you sure you want to delete?')"/>
						</form>
						<br>
						<p><a href="${pageContext.request.contextPath}/admin/category?page=${pagenumber}"> <button type="button" class="btn btn-block btn-dark">Back</button></a></p>
						</div>
				</div>
		</div>
	</div>
